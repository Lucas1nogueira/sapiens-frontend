...
export function ChangeDiscipline({ discipline }: Props) {
  ...
  const mutationUpdate = useMutation({
    mutationFn: (editedDiscipline: Discipline) =>
      updateDiscipline(discipline?.code as string, editedDiscipline),
    onSuccess: async () => {
  ...
  const mutationCreate = useMutation({
    ...
    onError: () => {
      enqueueNotification("Erro ao criar disciplina.", "error");
    },
  });
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    if (discipline) {
      ...
      mutationUpdate.mutate(editedDiscipline);
      return;
    }
    ...
    mutationCreate.mutate(newDiscipline);
  };
