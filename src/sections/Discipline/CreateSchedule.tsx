...
export function CreateSchedule({ discipline }: Props) {
  ...
  useMemo(() => {
    if (schedulesError || schedulesLoading) return [];

    if (!schedulesData) return [];

    const schedulesWithoutTempIds = schedulesData.map((schedule) => ({
      ...schedule,
      tempId: crypto.randomUUID(),
    }));

    setSchedules(schedulesWithoutTempIds);
  }, [schedulesData, schedulesError, schedulesLoading]);
  ...
  const mutationAddSchedules = useMutation({
    mutationFn: (schedules: Schedule[]) => {
      return saveSchedulesForDiscipline(discipline.code, schedules);
    },
    onSuccess: () => {
    ...
    onError: () => {
      enqueueNotification("Erro ao salvar horários.", "error");
    },
  });
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    mutationAddSchedules.mutate(schedulesWithoutTempIds);
  };
    ...
    <div className="max-h-[500px] overflow-y-auto p-4 bg-white">
      ...
      <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
        ...
        <Button
          ...
          isLoading={mutationAddSchedules.isPending}
        >
          Adicionar
        </Button>
      </form>
