...
export function TableAttendance({ lessons, sortedStudents }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all(allAttendancesPromises)
      .then((responses) => {
        const allAttendances = responses.flatMap((response) => response);
        ...
        // Criar um array de presenças cheias com as presenças existentes e novas
  ...
  const mutationManyAttendances = useMutation({
    mutationFn: saveManyAttendances,
    onSuccess: () =>
      enqueueNotification("Presenças salvas com sucesso!", "success"),
    onError: () => enqueueNotification("Erro ao salvar presenças.", "error"),
  });
  ...
  const handleSubmit = () => {
    ...
    mutationManyAttendances.mutate(attendancesToSave);
  };
  ...
  return (
    <div className="p-4">
      <Button
        ...
        className="mb-6 w-36"
        radius="sm"
        isLoading={mutationManyAttendances.isPending}
      >
        Salvar
      </Button>
